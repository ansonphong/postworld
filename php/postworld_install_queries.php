...
$pw_queries = array(
	...
	'FK'=> array(
		0 	=> array( // add fk from table user_meta to table wp_users
			...
			"query"	=> "ALTER TABLE `wp_postworld_a1`.`".$wpdb->pw_prefix.'user_meta'."` ADD CONSTRAINT `FK_wp_postworld_user_meta_wp_users` FOREIGN KEY (`user_id`) REFERENCES `wp_postworld_a1`.`wp_users` (`ID`) 
						ON DELETE cascade
	  					ON UPDATE NO ACTION;"
			)
		),
